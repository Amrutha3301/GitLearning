...
public List<EmployeeEntity> getEmployeesNativeQuery() throws Exception{
	Transaction ts=session.beginTransaction();
...
public List<EmployeeEntity> getEmployeeById(String eid1) throws Exception{
	Transaction ts=session.beginTransaction();
...
public List<EmployeeEntity> getEmployeeByFname(String fname1) throws Exception{
	Transaction ts=session.beginTransaction();
    Query q1= session.createQuery("From EmployeeEntity where fname=:fname");
    q1.setParameter("fname", fname1);
    List<EmployeeEntity> l1= q1.list();
	ts.commit();
	return l1;
}
